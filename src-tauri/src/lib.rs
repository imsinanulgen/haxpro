    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            .initialization_script(r#"
                ...
                // Reusable UI Function to show Navigator
                window.haxproShowNav = function() {
                ...
                // Inject UI natively directly to the screen on load
                window.addEventListener('DOMContentLoaded', function() {
                    let openBtn = document.createElement('div');
                    openBtn.id = 'haxpro-open-nav-btn';
                    openBtn.innerHTML = '🔗 Oda Değiştir';
                    openBtn.style.cssText = 'position:fixed;top:15px;left:15px;background:rgba(0,0,0,0.6);color:#fff;padding:8px 12px;border-radius:6px;cursor:pointer;font-family:sans-serif;font-size:13px;z-index:9999998;border:1px solid #444;transition:0.2s;user-select:none;';
                    openBtn.onmouseover = () => openBtn.style.background = 'rgba(0,0,0,0.9)';
                    openBtn.onmouseout = () => openBtn.style.background = 'rgba(0,0,0,0.6)';
                    openBtn.onclick = window.haxproShowNav;
